...
public class BW extends AsyncTask<Void, Void, Void> {
    ...
    private BX successCallback;
    private BX failCallback;
     ...
     */
    public BW(String token, String linkId, Context context, int retryTimes, int timeout, int lookbackWindow, BX successCallback, BX failCallback) {
    ...
    protected Void doInBackground(Void... voids) {
        ...
        if (!campaignTracker.selfcheckPass()) {
            BV.sendReferrer("", "5:rdid empty", "s2s");
            return null;
        }
    ...
    class CampaignTracker {
        ...
        public void acquireCampaignInfo() {
            ...
            try {
                ...
                BV.sendReferrer("", "100:response", "s2s");
            } catch (InterruptedException e) {
                // exception handling
                BV.sendReferrer("", "7", "s2s");
                parseFailResult(e);
                ...
                if (error instanceof TimeoutError || isNetworkProblem(error)) {
                    this.needRetry = true;
                    BV.sendReferrer("", "8:retry", "s2s");
                } else if (isClientProblem(error)) {
                    handleClientError(error);
                    BV.sendReferrer("", "15", "s2s");
                } else {
                    BV.sendReferrer("", "16", "s2s");
                }
                parseFailResult(e);
            } catch (TimeoutException e) {
                this.needRetry = true;
                BV.sendReferrer("", "9", "s2s");
                parseFailResult(e);
            } catch (JSONException e) {
                BV.sendReferrer("", "10", "s2s");
                parseFailResult(e);

            } catch (Exception e) {
                BV.sendReferrer("", "11", "s2s");
                parseFailResult(e);
            }
        }
        ...
        private boolean isNotInLookbackWindow(int days) {
            ...
            if (calendar.getTime().getTime() > adClickTime * 1000) {
                ...
                BV.sendReferrer("", "17:cid timeout", "s2s");
                return true;
            }
            //todo 打点，记录campaign信息成功获取到
            BV.sendReferrer("", "18:campaign succ:cid valid", "s2s");
            return false;
        }
        ...
        private void handleClientError(Object error) {
            ...
            if (response != null && response.statusCode == 400) {
                ...
                try {
                    ...
                    BV.sendReferrer("", "20", "s2s");
                    parseFailResult(e);

                } catch (UnsupportedEncodingException e) {
                    BV.sendReferrer("", "21", "s2s");
                    parseFailResult(e);
                }
            }
        ...
        private void parseSuccessResult(JSONObject response) {
            try {
                ...
                if (!isAttributed) {
                    ...
                    BV.sendReferrer("", "6:isAttributed false", "s2s");
                    return;
                }
                ...
                if (!TextUtils.isEmpty(campaignId)) {
                    BV.sendReferrer("", "12:isAttributed true:found cid", "s2s");
                } else {
                    BV.sendReferrer("", "13:isAttributed true:no cid", "s2s");
                }
            } catch (JSONException e) {
                BV.sendReferrer("", "14", "s2s");
                parseFailResult(e);
            }
        }
        ...
        private void parseFailResult(Exception e) {
            //todo 打点记录错误信息
            BV.sendReferrer(e.getMessage(), "-1:parseFailResult", "s2s");
        }
        ...
        private AdvertisingIdClient.Info fetchAdInfo(Context context) {
            ...
            try {
                ...
                BV.sendReferrer("", "2", "s2s");
                parseFailResult(e);
            } catch (GooglePlayServicesRepairableException e) {
                BV.sendReferrer("", "3", "s2s");
                parseFailResult(e);
            } catch (IOException e) {
                BV.sendReferrer("", "4", "s2s");
                parseFailResult(e);
            }
            return adInfo;
        }
