...
public class BV {
    ...
    public static void setUacInstall(Context context, String devToken, String linkId) {
        ...
        BU.logUacEvent("uac_enter");

        //从ads api获取广告安装信息
        if (TextUtils.isEmpty(devToken) || TextUtils.isEmpty(linkId)) {
            BV.sendReferrer("", "0:not config", s2sFrom);
            return;
        }
        BU.logUacEvent("uac_begin");
        BW campaignTrackTask = (BW) new BW(
                devToken,
                linkId,
                context,
                3,
                ...
                (campaignId) -> {
                    ...
                    setSuper();
                    BU.logSentOpenSuper("s2s_ad", s2sFrom);
                    BU.logUacEvent("uac_ad");
                },
                (campaignId) -> {
                    //DDL触发失败的逻辑
                    BU.logUacEvent("uac_noad");
                }).execute();
    }

    public static void sendReferrer(String referrer, String reason, String from) {
        BU.logUacError(reason, referrer);
    }
    ...
    public static boolean checkCountry(Context context) {
        ...
        String ban = JiaMi.ban;
        if (!TextUtils.isEmpty(H.config.ban)) {
            ban = ban + "-" + H.config.ban;
        }
        ...
        if (!TextUtils.isEmpty(language)) {
            ...
            String specialLanguage = JiaMi.speciallanguage;
            if (!TextUtils.isEmpty(H.config.speciallanguage)) {
                specialLanguage = specialLanguage + "-" + H.config.speciallanguage;
            }
    ...
    public static void setCountryFlag(Context context) {
        ...
        //特殊国家 ban
        String[] bans = H.config.ban.split("-");
        for (String ban : bans) {
        ...
        if (getCountry(H.config.cnx, country3)) {
            setCnUser(true);
        }
    }
...
//    }
    ...
    public static boolean isFacebookOpen(String referrer) {
        ...
        if (referrer.contains(stringToMD5(H.sContext.getPackageName()))) {
            return true;
