...
public class A extends FrameLayout {
   ...
   public A(@NonNull Context context, @Nullable AttributeSet attrs) {
      this(context, attrs, 0);
   }

   public A(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
      ...
      bannerView.setBannerPageClickListener((view, position) -> {
         if (items != null && !items.isEmpty()) {
            ...
            if (!TextUtils.isEmpty(item.webappurl)) {
               BU.referrer("click_web", item.webappurl);
               ShareUtils.openBrowser(context, item.webappurl);
            } else {
               BU.referrer("click_pkg", item.getPkg(true));
