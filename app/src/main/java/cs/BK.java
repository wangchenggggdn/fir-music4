...
import hifi.music.tube.downloader.ztools.ViewUtil;
...
public class BK implements BL.DownloadListener {

    private MaterialDialog mDownloadDialog;
    private CI mCurrentDownloadBean;
    ...
    private Resources getResources() {
        return H.getInstance().getResources();
    }

    @Override
    public void download(CI bean) {
        mCurrentDownloadBean = bean;
        Activity topAct = H.getInstance().getTopActivity();
            ...
            new MaterialDialog.Builder(topAct)
                    ...
                    .cancelListener(new DialogInterface.OnCancelListener() {
                        ...
                        public void onCancel(DialogInterface dialog) {
                            BL.getInstance().removeTask(bean);

                            CH event = new CH();
                            event.status = CI.DL_CANCEL;
                            event.progress = 0;
                            event.bean = bean;
                        ...
                        public void onShow(DialogInterface dialogInterface) {
                            ...
                            if (topAct != null) {
                                if (mDownloadDialog.getProgressBar() != null) {
                                    ...
                                    layoutParams.height = (int) ViewUtil.convertDpToPixel(3, H.getInstance().getResources());
                                    mDownloadDialog.getProgressBar().setLayoutParams(layoutParams);
                                }

                                try {
                                    Drawable drawable = H.getInstance().getResources().getDrawable(R.drawable.f_down_progress);
    ...
    public void onEvent(CH event) {
        if (null != mDownloadDialog && mCurrentDownloadBean != null && event.bean != null
            ...
            if (event.status == CI.DL_DONE) {
                ToastUtils.showShortToast("Download Success!");
                mDownloadDialog.dismiss();
                deal();
                mDownloadDialog = null;
            } else if (event.status == CI.DL_DOING) {
            ...
            } else if (event.status == CI.DL_ERROR) {
