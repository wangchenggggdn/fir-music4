...
public class L extends BM {
    ...
    public static void start(Context context, ArrayList<BZ> genres) {
        Intent starter = new Intent(context, L.class);
    ...
    protected void initDatas() {
        ArrayList<BZ> genres = getIntent().getParcelableArrayListExtra(KEY_GENRES);
    ...
    class GenresAdapter extends RecyclerView.Adapter<GenresAdapter.Holder> {
        ...
        GenresAdapter(ArrayList<BZ> genres) {
            this.genres = genres;
        }
        ...
        public void onBindViewHolder(@NonNull Holder holder, int pos) {
            BZ bean = genres.get(pos);
            ImageHelper.loadMusic(holder.iv, bean.image, L.this, 0, 0);
    ...
    private void onItemClick(BZ bean) {
        J.start(this, bean);
    }
