...
public class J extends BM implements AMusicAdapter.BindCallback {
    ...
    public static void start(Context context, ArrayList<CI> list) {
        Intent starter = new Intent(context, J.class);
    ...
    private List<CI> musics = new ArrayList<>();
    ...
    protected void initDatas() {
        if (workMode == WORK_MODE_DAILY_PICK) {
            ArrayList<CI> list = getIntent().getParcelableArrayListExtra(KEY_DATAS);
            musics.addAll(list);
        } else {
            loadGenre();
        }
