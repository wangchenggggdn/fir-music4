...
public class BR extends CA {
    ...
    public List<CI> songs = new ArrayList<>();


    public static final int NO_POSITION = -1;
    private BQ playMode = BQ.LOOP;
     ...
     * - current play mode is BQ.LIST (the only limited play mode)
    ...
    public boolean hasNext(boolean fromComplete) {
        ...
        if (fromComplete) {
            if (playMode == BQ.LIST && playingIndex + 1 >= songs.size()) return false;
        }
        return true;
    }
