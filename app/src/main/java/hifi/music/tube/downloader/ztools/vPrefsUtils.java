...
public class vPrefsUtils {
    ...
    public static Y getRateBean() {
        SharedPreferences s = getPrefs();
        if (TextUtils.isEmpty(s.getString(RATE_BEAN, ""))) {
            return null;
        }
        return new Gson().fromJson(s.getString(RATE_BEAN, ""), Y.class);
    }

    public static void setRateBean(Y bean) {
        SharedPreferences s = getPrefs();
    ...
    public static List<CI> getDownloadCache() {
        SharedPreferences s = getPrefs();
        if (TextUtils.isEmpty(s.getString(DOWNLOAD, ""))) {
            return null;
        }
        return new Gson().fromJson(s.getString(DOWNLOAD, ""), new TypeToken<List<CI>>() {
        }.getType());
    }

    private static Gson gson = new Gson();

    public synchronized static void setDownloadCache(List<CI> list) {
        SharedPreferences s = getPrefs();
        try {
    ...
    private static SharedPreferences getPrefs() {
        return PreferenceManager.getDefaultSharedPreferences(H.getInstance());
    }

    public static BQ lastPlayMode() {
        String playModeName = getPrefs().getString(PLAY_MODE, null);
        if (playModeName != null) {
            return BQ.valueOf(playModeName);
        }
        return BQ.getDefault();
    }

    public static void setPlayMode(BQ playMode) {
