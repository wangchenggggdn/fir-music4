...
public class HttpArchiveMusic {
    ...
    public static void search(String word, SearchListener searchListener) {
        ...
        Observable.create(new ObservableOnSubscribe<List<CI>>() {
            @Override
            public void subscribe(@NotNull ObservableEmitter<List<CI>> emitter) throws Exception {
                ...
                for (Element itemElement : listItemElements) {
                    ...
                    CI bean = new CI();
                    bean.channel = CI.CHANNEL_ARCHIVE;
        ...
        }).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(new Observer<List<CI>>() {
            @Override
            ...
            @Override
            public void onNext(@NotNull List<CI> list) {
