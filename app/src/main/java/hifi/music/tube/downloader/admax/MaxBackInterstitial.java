...
public class MaxBackInterstitial implements MaxAdListener, MaxAdRevenueListener {
    ...
    public boolean isReady() {
        return H.config.ad && interstitialAd != null && interstitialAd.isReady() && (H.config.se_pop <= 0 || vPrefsUtils.seNextLong() % H.config.se_pop == 0);
    }

    public void show() {
        if (H.config.ad && interstitialAd != null && interstitialAd.isReady()) {
