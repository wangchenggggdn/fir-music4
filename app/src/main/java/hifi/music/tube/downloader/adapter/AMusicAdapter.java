...
import hifi.music.tube.downloader.dialog.QualityDialog;
...
import hifi.music.tube.downloader.network.yt.YTAudioBean;
import hifi.music.tube.downloader.network.yt.YTGetAudios;
...
import cj.BS;
import ft.E;
import hifi.music.tube.downloader.ztools.AdRewardedDialog;
...
public class AMusicAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    private List<CI> mDatas;
    private Activity mActivity;
    private BindCallback bindCallback;

    public AMusicAdapter(Activity activity, List<CI> datas) {
        mActivity = activity;
        mDatas = datas;
    }

    public void setDatas(List<CI> datas) {
        mDatas = datas;
        notifyDataSetChanged();
    }

    public void addDatas(List<CI> datas) {
        if (mDatas == null) {
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, final int position) {
        ...
        if (holder instanceof VideoSmallHolder) {
            final CI track = (CI) mDatas.get(position);
...
//            }
                ...
                View.OnClickListener playClick = new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        ...
                        if (bean.channel == CI.CHANNEL_YT || bean.channel == CI.CHANNEL_SOUND) {
                            if (TextUtils.isEmpty(bean.getDownloadUrl())) {
                                handleYT_SoundUrl(mActivity, bean, true);
                            } else {
                                BS.playSingle(bean);
                            }
                        } else if (bean.channel == CI.CHANNEL_MP3juice) {
                            String id = bean.id;
...
//                        } else if (bean.channel == CI.CHANNEL_QQ) {
//                            if (TextUtils.isEmpty(bean.getDownloadUrl())) {
//                                String id = bean.id;
...
//                                BS.playSingle(bean);
//                            }
//                        } else if (bean.channel == CI.CHANNEL_WYY) {
//                            if (TextUtils.isEmpty(bean.getDownloadUrl())) {
//                                String id = bean.id;
...
//                                BS.playSingle(bean);
//                            }
                        } else if (bean.channel == CI.CHANNEL_NHAC) {
                            if (TextUtils.isEmpty(bean.getDownloadUrl())) {
    ...
    public static void tryDownload(Activity activity, CI bean) {
        MaxDownloadInterstitial.getInstance().loadInterstitialAd(activity);

        if (H.config.ad && !Z.getInstance().isShowRate() && K.notRewardOrRunOut()) {
    ...
    private static void doDownload(Activity activity, CI bean) {
        if (bean.channel == CI.CHANNEL_YT || bean.channel == CI.CHANNEL_SOUND) {
            if (TextUtils.isEmpty(bean.getDownloadUrl())) {
                handleYT_SoundUrl(activity, bean, false);
            } else {
                BL.getInstance().download(bean, activity);
            }
//        } else if (bean.channel == CI.CHANNEL_QQ) {
//            if (TextUtils.isEmpty(bean.getDownloadUrl())) {
//                String id = bean.id;
...
//            }
    ...
    private static void handleYT_SoundUrl(Activity activity, CI bean, boolean play) {
        ...
        if (bean.channel == CI.CHANNEL_YT) {
            YTManager.getInstance().getAudios(bean.listenUrl, listener);
        } else if (bean.channel == CI.CHANNEL_SOUND) {
            YTManager.getInstance().getAudiosSound(bean.listenUrl, listener);
        }
    }

    private static void doneHandleUrl(Activity activity, CI bean, boolean play, String dUrl, String lUrl) {
    ...
    public class VideoSmallHolder extends RecyclerView.ViewHolder {
        ...
        public VideoSmallHolder(View itemView) {
            ...
            durationLy = itemView.findViewById(R.id.duration_ll);
            durationTv = itemView.findViewById(R.id.duration_tv);
            downloadIv = itemView.findViewById(R.id.download_iv);
            playIv = itemView.findViewById(R.id.play_iv);
            nativeAdLayout = itemView.findViewById(R.id.native_ad);
            itemContentLayout = itemView.findViewById(R.id.item_content_layout);
        }
    }
